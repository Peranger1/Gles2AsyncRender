...
{
    RuntimeDiagnostics::logInfo("[WinAngleTextureWriter]", message);
}

void logPublishWarning(const QString &message)
{
    RuntimeDiagnostics::logWarning("[WinAngleTextureWriter]", message);
}

void logPublishDiag(const QString &message)
{
    RuntimeDiagnostics::logDiag("[WinAngleTextureWriter]", message);
}
}

struct WinAngleTextureWriter::Impl final
{
    ...
    {
        if (publishFramebufferId != 0U || readbackFramebufferId != 0U || program != 0U) {
            return true;
        }

        for (const ImportedSlot &slot : importedSlots) {
            if (slot.surface != EGL_NO_SURFACE || slot.textureId != 0U || slot.keyedMutex) {
                return true;
            }
        }
    ...
    {
        WinAngleRuntime *activeRuntime = runtime();
        return activeRuntime ? activeRuntime->procTable() : nullptr;
    }
    ...
    bool initialize(QString *error)
    {
        ...
        WinAngleRuntime *activeRuntime = runtime();
        if (activeRuntime == nullptr || slotPool == nullptr) {
            if (error) {
                *error = QStringLiteral("WinAngleTextureWriter requires both a runtime and texture slots.");
            }
            return false;
        }

        const Gles2ProcTable *procTable = gl();
        if (procTable == nullptr || !procTable->isValid()) {
            if (error) {
                *error = QStringLiteral("WinAngleTextureWriter requires a valid GLES2 proc table.");
            }
            return false;
        }
        ...
        logPublishDiag(QStringLiteral("[diag] Writer initialize currentContext=%1 currentDisplay=%2 runtimeDisplay=%3")
                           .arg(quintptr(procTable->eglGetCurrentContext ? procTable->eglGetCurrentContext() : EGL_NO_CONTEXT), 0, 16)
                           .arg(quintptr(procTable->eglGetCurrentDisplay ? procTable->eglGetCurrentDisplay() : EGL_NO_DISPLAY), 0, 16)
                           .arg(quintptr(activeRuntime->eglDisplay()), 0, 16));

        procTable->glGenFramebuffers(1, &publishFramebufferId);
        procTable->glGenFramebuffers(1, &readbackFramebufferId);
        if (publishFramebufferId == 0U || readbackFramebufferId == 0U) {
            if (error) {
                *error = QStringLiteral("Failed to create WinAngleTextureWriter framebuffers. currentContext=%1 currentDisplay=%2 glError=%3")
                             .arg(quintptr(procTable->eglGetCurrentContext ? procTable->eglGetCurrentContext() : EGL_NO_CONTEXT), 0, 16)
                             .arg(quintptr(procTable->eglGetCurrentDisplay ? procTable->eglGetCurrentDisplay() : EGL_NO_DISPLAY), 0, 16)
                             .arg(glErrorHex(procTable->glGetError()));
            }
            return false;
        }
...
void main()
{
    gl_FragColor = texture2D(uTexture, vTexCoord);
        ...
        if (!Gles2ShaderUtils::buildProgram(*procTable, kVertexShader, kFragmentShader, &program, error)) {
            return false;
        }

        positionLocation = procTable->glGetAttribLocation(program, "aPosition");
        texCoordLocation = procTable->glGetAttribLocation(program, "aTexCoord");
        samplerLocation = procTable->glGetUniformLocation(program, "uTexture");
        if (positionLocation < 0 || texCoordLocation < 0 || samplerLocation < 0) {
            if (error) {
                *error = QStringLiteral("The WinAngleTextureWriter shader program is missing required attributes or uniforms.");
            }
            return false;
        }
        ...
        const bool angleImportEntryPoints = procTable->supportsAngleD3DTextureImport();
        const bool hasDisplay = activeRuntime->eglDisplay() != EGL_NO_DISPLAY;
        const bool hasConfig = activeRuntime->eglConfig() != nullptr;
        ...
        logPublishMessage(QStringLiteral("WinAngleTextureWriter ready. requestedMode=%1 initialPath=%2 angleImportEntryPoints=%3 eglDisplay=%4 eglConfig=%5 reason=%6")
                              .arg(publishModeToString(publishMode))
                              .arg(gpuPublishEnabled ? QStringLiteral("gpu") : QStringLiteral("cpu"))
                              .arg(angleImportEntryPoints)
                              .arg(quintptr(activeRuntime->eglDisplay()), 0, 16)
                              .arg(quintptr(activeRuntime->eglConfig()), 0, 16)
                              .arg(initReason));
        initialized = true;
        return true;
    }
                       ...
                       QString *error)
    {
        if (runtime() == nullptr || slotPool == nullptr) {
            if (error) {
                *error = QStringLiteral("WinAngleTextureWriter is not initialized.");
            }
            return false;
        }
        if (sourceTextureId == 0U) {
            if (error) {
                *error = QStringLiteral("WinAngleTextureWriter requires a valid source texture.");
            }
            return false;
        }
    ...
    void releaseGlResources()
    {
        ...
        if (readbackFramebufferId != 0U) {
            procTable->glDeleteFramebuffers(1, &readbackFramebufferId);
            readbackFramebufferId = 0U;
        }

        Gles2ShaderUtils::deleteProgram(*procTable, &program);
        positionLocation = -1;
        texCoordLocation = -1;
        samplerLocation = -1;
    }
    ...
    bool ensureSlotResources(int slotIndex, const QSize &size, QString *error)
    {
        if (slotIndex < 0 || slotIndex >= slotResources.size()) {
            if (error) {
                *error = QStringLiteral("WinAngleTextureWriter slot index is out of range.");
            }
            return false;
        }
        ...
        WinAngleRuntime *activeRuntime = runtime();
        ID3D11Device *device = activeRuntime ? activeRuntime->d3d11Device() : nullptr;
        if (device == nullptr) {
            if (error) {
                *error = QStringLiteral("WinAngleTextureWriter D3D11 device is unavailable.");
            }
            return false;
        }
    ...
    bool ensureImportedSlot(int slotIndex, const QSize &size, QString *error)
    {
        ...
        if (slotIndex < 0 || slotIndex >= importedSlots.size()) {
            if (error) {
                *error = QStringLiteral("WinAngleTextureWriter import slot index is out of range.");
            }
            return false;
        }

        D3D11SharedTextureSlotInfo slotInfo;
        if (!slotPool->querySlot(slotIndex, &slotInfo)) {
            if (error) {
                *error = QStringLiteral("WinAngleTextureWriter import slot %1 is unavailable.").arg(slotIndex);
            }
            return false;
        }
        ...
        if (procTable == nullptr) {
            if (error) {
                *error = QStringLiteral("WinAngleTextureWriter GLES2 proc table is unavailable.");
            }
            return false;
        }

        slot.surface = procTable->eglCreatePbufferFromClientBuffer(activeRuntime->eglDisplay(),
                                                                   EGL_D3D_TEXTURE_2D_SHARE_HANDLE_ANGLE,
                                                                   reinterpret_cast<EGLClientBuffer>(slotInfo.sharedHandle),
                                                                   activeRuntime->eglConfig(),
                                                                   surfaceAttributes);
        if (slot.surface == EGL_NO_SURFACE) {
            if (error) {
                *error = QStringLiteral("eglCreatePbufferFromClientBuffer(slot %1) failed with EGL error %2")
                             .arg(slotIndex)
                             .arg(glErrorHex(procTable->eglGetError()));
            }
            return false;
        }

        void *keyedMutexPtr = nullptr;
        if (procTable->eglQuerySurfacePointerANGLE(activeRuntime->eglDisplay(), slot.surface, EGL_DXGI_KEYED_MUTEX_ANGLE, &keyedMutexPtr) != EGL_TRUE
        ...
        procTable->glGenTextures(1, &slot.textureId);
        procTable->glBindTexture(GL_TEXTURE_2D, slot.textureId);
    ...
    {
        WinAngleRuntime *activeRuntime = runtime();
        if (activeRuntime == nullptr || slotIndex < 0 || slotIndex >= importedSlots.size()) {
            return;
        }
        ...
        if (slot.textureId != 0U) {
            procTable->glDeleteTextures(1, &slot.textureId);
            slot.textureId = 0U;
        }
        slot.keyedMutex.Reset();
        if (slot.surface != EGL_NO_SURFACE && procTable->eglDestroySurface != nullptr) {
            procTable->eglDestroySurface(activeRuntime->eglDisplay(), slot.surface);
                                ...
                                QString *error)
    {
        ...
        if (!slot.keyedMutex || slot.surface == EGL_NO_SURFACE || slot.textureId == 0U) {
            if (error) {
                *error = QStringLiteral("WinAngleTextureWriter imported slot is not ready.");
            }
            return false;
        }
        ...
        procTable->glBindTexture(GL_TEXTURE_2D, slot.textureId);
        if (procTable->eglBindTexImage(activeRuntime->eglDisplay(), slot.surface, kBackBuffer) != EGL_TRUE) {
            procTable->glBindTexture(GL_TEXTURE_2D, 0);
        ...
        procTable->glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, slot.textureId, 0);
        ...
        if (status != GL_FRAMEBUFFER_COMPLETE) {
            ...
            procTable->eglReleaseTexImage(activeRuntime->eglDisplay(), slot.surface, kBackBuffer);
            procTable->glBindTexture(GL_TEXTURE_2D, 0);
            slot.keyedMutex->ReleaseSync(0);
            if (error) {
                *error = QStringLiteral("The WinAngleTextureWriter imported framebuffer is incomplete: 0x%1")
                             .arg(unsigned(status), 0, 16);
            }
            return false;
        }

        procTable->glViewport(0, 0, sourceSize.width(), sourceSize.height());
        ...
        const EGLBoolean releaseImageOk = procTable->eglReleaseTexImage(activeRuntime->eglDisplay(), slot.surface, kBackBuffer);
        procTable->glBindTexture(GL_TEXTURE_2D, 0);
    ...
    {
        const Gles2ProcTable *procTable = gl();
        if (procTable == nullptr || readbackFramebufferId == 0U) {
            if (error) {
                *error = QStringLiteral("The WinAngleTextureWriter readback framebuffer is not initialized.");
            }
            return false;
        }

        procTable->glBindFramebuffer(GL_FRAMEBUFFER, readbackFramebufferId);
        procTable->glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, sourceTextureId, 0);
        const GLenum status = procTable->glCheckFramebufferStatus(GL_FRAMEBUFFER);
        if (status != GL_FRAMEBUFFER_COMPLETE) {
            procTable->glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, 0, 0);
            procTable->glBindFramebuffer(GL_FRAMEBUFFER, 0);
            if (error) {
                *error = QStringLiteral("The WinAngleTextureWriter readback framebuffer is incomplete: 0x%1")
                             .arg(unsigned(status), 0, 16);
            }
            return false;
        }
        ...
        procTable->glPixelStorei(GL_PACK_ALIGNMENT, 1);
        procTable->glReadPixels(0,
                                0,
                                sourceSize.width(),
                                sourceSize.height(),
                                GL_RGBA,
                                GL_UNSIGNED_BYTE,
                                readbackBytes.data());
        const GLenum readbackError = procTable->glGetError();
        procTable->glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, 0, 0);
        procTable->glBindFramebuffer(GL_FRAMEBUFFER, 0);
        if (readbackError != GL_NO_ERROR) {
            if (error) {
                *error = QStringLiteral("glReadPixels failed in WinAngleTextureWriter: %1").arg(glErrorHex(readbackError));
            }
            return false;
        }
    ...
    bool uploadPixels(int slotIndex, const QSize &sourceSize, QString *error)
    {
        ...
        if (!slot.texture || !slot.keyedMutex) {
            if (error) {
                *error = QStringLiteral("The WinAngleTextureWriter destination slot is not ready.");
            }
            return false;
        }

        WinAngleRuntime *activeRuntime = runtime();
        ID3D11DeviceContext *deviceContext = activeRuntime ? activeRuntime->d3d11DeviceContext() : nullptr;
        if (deviceContext == nullptr) {
            if (error) {
                *error = QStringLiteral("The WinAngleTextureWriter D3D11 device context is unavailable.");
            }
            return false;
        }
