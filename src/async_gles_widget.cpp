...
void AsyncGlesWidget::initializeGL()
{
    ...
    m_worker = std::make_unique<SharedTextureWorker>(context(), m_surface.get(), context()->format(), this);
...
void AsyncGlesWidget::onTextureReady(quint32 textureId, QSize size, quint64 frameIndex)
{
    if (!m_acceptFrames) {
        return;
    }

    m_displayTexture = textureId;
    m_displayTextureSize = size;
    m_displayFrame = frameIndex;
    update();
}
